...
<body class="text-black">
    <x-navbar-auth />
    <div class="flex justify-center items-center mt-14 mb-8 md:mb-20">
        <div class="flex-col p-5 w-11/12 mt-5 md:w-3/4 md:shadow-2xl shadow-gray px-7">
            ...
            <form action="{{ route('change.nik.action', ['id'=>$userRequest]) }}" method="post">
                @csrf
                @method('PUT')
                <div class="pt-4">
                    <label for="" class="mb-0.5">NIK</label><br>
                    <input type="text" name="nik" value="{{ $nikChanged }}" class="w-full px-2 py-1 pb-2 border border-gray rounded-md" readonly required>
                    @error('nik')
                        <p>{{ $message }}</p>
                    @enderror
                </div>
